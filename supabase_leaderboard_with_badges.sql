...
BEGIN
  ...
  WITH relevant_users AS (
    ...
    UNION
    SELECT auth.uid() as u_id FROM profiles WHERE id = auth.uid() AND scope = 'friends'
    UNION
    SELECT id as u_id FROM profiles WHERE scope = 'global'
  ),
  user_badges_count AS (
    SELECT ub.user_id, COUNT(ub.id) as b_count
    FROM user_badges ub
    GROUP BY ub.user_id
  )
  SELECT 
    ...
    COALESCE(ubc.b_count, 0)::BIGINT as badge_count,
  ...
  LEFT JOIN user_badges_count ubc ON ru.u_id = ubc.user_id
  GROUP BY ru.u_id, p.display_name, ubc.b_count
